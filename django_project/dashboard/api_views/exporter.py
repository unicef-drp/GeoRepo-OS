...
from dashboard.serializers.export_request import (
    ...
    DatasetExportRequestItemSerializer,
    DatasetExportRequestDetailSerializer
)
from georepo.api_views.dataset import DatasetExportBase
from georepo.api_views.dataset_view import DatasetViewExportBase
from dashboard.api_views.common import DatasetReadPermission
from dashboard.serializers.tiling_config import TilingConfigSerializer
...
class DatasetExportHistoryList(
    AzureAuthRequiredMixin, DatasetReadPermission, APIView
):
    """
    API to fetch list of Dataset export request.
    """
    permission_classes = [IsAuthenticated]

    def get(self, *args, **kwargs):
        dataset = self.get_dataset()
        export_requests = DatasetExportRequest.objects.filter(
            dataset=dataset
        )
        if not self.request.user.is_superuser:
            export_requests = export_requests.filter(
...
class DatasetExportRequestDetail(
    AzureAuthRequiredMixin, DatasetReadPermission,
    DatasetExportBase, APIView
):
    """
    API to create and fetch export request detail.
    """
    ...
    def get(self, *args, **kwargs):
        ...
        export_request = get_object_or_404(
            DatasetExportRequest, id=request_id
        )
        return Response(
            status=200,
            data=DatasetExportRequestDetailSerializer(export_request).data
        )
...
class ExportRequestDatasetMetadata(
    DatasetReadPermission, APIView
):
    """
    API to fetch metadata of Dataset for requesting export data.
    """
    permission_classes = [IsAuthenticated]
    ...
    def get(self, *args, **kwargs):
        ...
        return Response(
            ...
            data={
                ...
                'available_formats': formats,
                'is_simplification_available': (
                    dataset.is_simplified
                ),
                'current_simplification_status': (
                    dataset.simplification_sync_status
                ),
                'tiling_configs': TilingConfigSerializer(
                    tiling_configs, many=True
                ).data
            }
        )
