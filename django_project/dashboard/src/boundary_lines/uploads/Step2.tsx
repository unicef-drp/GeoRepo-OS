        ...
        <Grid item className="Step2Content">
          { uploads.map( (upload, index) => (
            <TabPanel value={props.initChildTab} index={index} key={index}>
              <Step2LevelUpload uploadData={upload} updateLeveData={updateLevelData}
                 onBackClicked={props.onBackClicked} setFormIsDirty={props.setFormIsDirty}
                 isReadOnly={props.isReadOnly} canResetProgress={props.canResetProgress} onResetProgress={props.onResetProgress} />
            </TabPanel>
            )
          )}
