...
def dataset_sql_query(dataset_id, level, tolerance=None):
    ...
    ids = EntityId.objects.filter(
        geographical_entity__dataset__id=dataset_id,
    ...
    # group by label of the name distinct
...
def create_configuration_files(dataset: Dataset) -> List[str]:
    """
    ...
    # get list of entity type in dataset
    entity_levels = dataset.geographicalentity_set.order_by(
        'level'
    ).values_list('level', flat=True).distinct('level')
    ...
    for dataset_conf in dataset_tiling_conf:
        ...
        adminlevel_confs = dataset_conf.adminleveltilingconfig_set.all()
        toml_data['maps'] = [{
            'name': f'temp_{str(dataset.uuid)}',
            'layers': []
        }]
        admin_levels = []
        for adminlevel_conf in adminlevel_confs:
            ...
            sql = dataset_sql_query(
                dataset.id,
                level,
                tolerance=adminlevel_conf.simplify_tolerance
            )
            provider_layer = {
                'name': f'Level-{level}',
...
def generate_vector_tiles(dataset: Dataset, overwrite: bool = False):
    from georepo.utils import generate_geojson, generate_shapefile
    dataset.tiling_status = Dataset.DatasetTilingStatus.PROCESSING
    dataset.tiling_start_date = datetime.now()
    dataset.tiling_end_date = None
    dataset.tiling_progress = 0
    ...
    for toml_config_file in toml_config_files:
        ...
        processed_count += 1
        dataset.tiling_progress = (
            (100 * processed_count) / len(toml_config_files)
        )
        dataset.save(update_fields=['tiling_progress'])
    dataset_vectortiles_dir = str(dataset.uuid)
    original_vector_tile_path = os.path.join(
        settings.LAYER_TILES_PATH,
        dataset_vectortiles_dir
    )
    ...
    shutil.move(
        os.path.join(
            settings.LAYER_TILES_PATH,
            f'temp_{dataset_vectortiles_dir}'
        ),
        original_vector_tile_path
    )

    dataset.vector_tiles_path = (
        f'/layer_tiles/{dataset_vectortiles_dir}/{{z}}/{{x}}/{{y}}'
        f'?t={int(time.time())}'
    )
    dataset.tiling_status = Dataset.DatasetTilingStatus.DONE
    dataset.tiling_end_date = datetime.now()
    dataset.tiling_progress = 100
    dataset.save(update_fields=[
        'vector_tiles_path',
        'tiling_status',
        'tiling_end_date',
    ...
    logger.info('Extracting shapefile from {}'.format(dataset.label))
