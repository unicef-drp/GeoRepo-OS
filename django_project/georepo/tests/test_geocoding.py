...
class TestProcessGeocodingRequest(BaseDatasetViewTest):
    ...
    def test_submit_batch_geocoding_nearest_concept_uuid(self, mocked_task):
        ...
        with geocoding_request.output_file.open('rb') as json_data:
            ...
            self.assertEqual(len(features['features']), 3)
            feat_1 = features['features'][0]
            self.assertIn('concept_uuid', feat_1['properties'])
            self.assertEqual(len(feat_1['properties']['concept_uuid']), 1)
            self.assertEqual(feat_1['properties']['concept_uuid'][0],
                             str(self.pak0_2.uuid))
            feat_2 = features['features'][1]
            self.assertIn('concept_uuid', feat_2['properties'])
            self.assertEqual(len(feat_2['properties']['concept_uuid']), 1)
            self.assertEqual(feat_2['properties']['concept_uuid'][0],
                             str(self.pak0_2.uuid))
            feat_3 = features['features'][2]
            self.assertIn('concept_uuid', feat_3['properties'])
            self.assertEqual(len(feat_3['properties']['concept_uuid']), 1)
            self.assertEqual(feat_3['properties']['concept_uuid'][0],
                             str(self.pak0_2.uuid))
